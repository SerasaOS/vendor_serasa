function __print_serasa_functions_help() {
cat <<EOF
Additional SerasaOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- serasagerrit:   A Git wrapper that fetches/pushes patch from/to SerasaOS Gerrit Review.
- serasarebase:   Rebase a Gerrit change and push it again.
- serasaremote:   Add git remote for SerasaOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for SerasaOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Serasa model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch serasa_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/serasa-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.serasa.device | grep -q "$SERASA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SERASA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function serasaremote()
{
    ...
    git remote rm serasa 2> /dev/null
    ...
    local SERASA_USER=$(git config --get review.review.serasaos.org.username)
    if [ -z "$SERASA_USER" ]
    then
        git remote add serasa ssh://review.serasaos.org:29418/$PFX$PROJECT
    else
        git remote add serasa ssh://$SERASA_USER@review.serasaos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'serasa' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SerasaOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.serasa.device | grep -q "$SERASA_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        serasaremote
        git push serasa HEAD:refs/heads/'$1'
    fi
    '
}

function serasagerrit() {
    ...
    local user=`git config --get review.review.serasaos.org.username`
...
function serasarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SerasaOS Gerrit Rebase Usage: "
        echo "      serasarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.serasaos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.serasa.device | grep -q "$SERASA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/serasa/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SERASA_FIXUP_COMMON_OUT ]; then
