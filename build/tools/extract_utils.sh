...
function setup_vendor() {
    ...
    export SERASA_ROOT="$3"
    if [ ! -d "$SERASA_ROOT" ]; then
        echo "\$SERASA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SERASA_ROOT/$OUTDIR" ]; then
        mkdir -p "$SERASA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SERASA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SERASA_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SERASA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SERASA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
    ...
    if [ -z "$PATCHELF" ]; then
        export PATCHELF="$SERASA_ROOT"/prebuilts/tools-serasa/${HOST}-x86/bin/patchelf
    fi
}
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SERASA_ROOT"/prebuilts/tools-serasa/common/smali/baksmali.jar
        export SMALIJAR="$SERASA_ROOT"/prebuilts/tools-serasa/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SERASA_ROOT"/prebuilts/tools-serasa/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SERASA_ROOT"/prebuilts/tools-serasa/${HOST}-x86/bin/compact_dex_converter
    fi
...
=======
>>>>>>> dab8f509 (fixup! extract_utils: implement patchelf)
    ...
    if [ ! -f "$SERASA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SERASA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SERASA_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SERASA_ROOT"/vendor/serasa/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SERASA_ROOT"/"$OUTDIR"/radio
