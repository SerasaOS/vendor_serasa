...
func init() {
	android.RegisterModuleType("serasa_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = serasaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = serasaExpandVariables(ctx, dep_file)
	...
	cmd := serasaExpandVariables(ctx, String(g.properties.Cmd))
